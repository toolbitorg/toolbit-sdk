...
I2cHw::I2cHw(TbiService *tbisrv, int base)
{
	...
	mAttI2cRw3Byte = new Attribute(base + ATT_I2C_RW_3BYTE, 0x00, 0x00);
	mAttI2cRw4Byte = new Attribute(base + ATT_I2C_RW_4BYTE, 0x00, 0x00);
}
...
uint32_t I2cHw::read3byte(uint8_t reg_addr)
{
	bool status;
	...
	status = mTbiSrv->readAttribute(mAttI2cRw3Byte);
	if (status) {
		// error
		return 0;
	}
	return 0x00FFFFFF & mAttI2cRw3Byte->getValueInt32();
}
